...
public class Tetris extends Applet 
{
	...
	private final static int deleted_rows = 5;
	...
	private Grid game_grid = new Grid(grid, true);
	private Grid next_piece_canvas = new Grid(next_piece_grid, false);
	private Timer timer;
	...
	private class TetrisPiece 
	{
		...
		public boolean canPaste() 
		{
			for(int i=0; i<4; i++) 
			{
				...
				if(squares[i][j]) 
				{
					if(0 > to_x || to_x >= columns|| to_y >= rows) 
						{
							return false;
						}
	...
	private class Grid extends DoubleBufferedCanvas 
	{
		private int grid[][];
		private boolean paint_background;
		public Grid(int[][] grid, boolean do_background) 
		{
			this.grid = grid;
		...
		public void paint(Graphics g) 
		{
			...
			if(paint_background) 
			{
				g.setColor(background_colors[(num_rows_deleted / deleted_rows) % background_colors.length]);
	...
	private void removeFullRows() 
	{
		...
		if(num_rows_deleted / deleted_rows != (num_rows_deleted+n_full) / deleted_rows) 
		{
			timer.faster();
			level_label.addValue(n_full / deleted_rows + 1);
			level_label.repaint();
		}
	...
	public void init() 
	{
		...
		control.add(start_newgame_butt);
		control.add(pause_resume_butt);
		control.setBackground(background_color);
		right_panel.add(control);
...
class DoubleBufferedCanvas extends Canvas 
{
	...
	public Graphics startPaint (Graphics sysgraph) 
	{
		...
		Dimension d = getSize();
		if ((mActiveOffscreenImage == null)||(d.width != mOffscreenSize.width)||(d.height != mOffscreenSize.height)) 
		{
